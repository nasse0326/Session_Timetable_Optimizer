...
export default function OptimizationAdModal({ isOpen, onComplete }: OptimizationAdModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 light:bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 light:bg-white border border-slate-700/80 light:border-slate-200 rounded-3xl p-6 sm:p-7 max-w-lg w-full shadow-2xl relative overflow-hidden flex flex-col gap-5 animate-in zoom-in-95 duration-200">
        {/* 背景の装飾グロー */}
        ...
        <div className="space-y-3 relative z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-slate-100 light:text-slate-900 text-sm sm:text-base flex items-center gap-2">
                  <span>曲順を自動最適化中</span>
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-indigo-500/20 light:bg-indigo-100 text-indigo-300 light:text-indigo-700 border border-indigo-500/30 light:border-indigo-300">
                    {Math.round(progress)}%
                  </span>
                </h3>
                <p className="text-xs text-slate-400 light:text-slate-500 truncate max-w-[260px] sm:max-w-[340px]">
                  {statusMessage}
                </p>
              </div>
            </div>
            ...
            <button
              type="button"
              onClick={onComplete}
              className="text-xs text-slate-400 light:text-slate-500 hover:text-slate-200 light:hover:text-slate-800 transition-colors px-2.5 py-1 rounded-lg hover:bg-slate-800/80 light:hover:bg-slate-100 border border-transparent hover:border-slate-700 light:hover:border-slate-200 font-medium"
            >
              スキップ
            </button>
          </div>
          ...
          {/* プログレスバー */}
          <div className="w-full bg-slate-950 light:bg-slate-100 rounded-full h-2.5 overflow-hidden p-0.5 border border-slate-800 light:border-slate-200 shadow-inner">
            <div
        ...
        <div className="relative z-10 bg-slate-950/70 light:bg-slate-50 border border-slate-800/90 light:border-slate-200 rounded-2xl p-4 shadow-xl">
          <div className="flex items-center justify-between text-[11px] font-semibold text-slate-500 light:text-slate-400 mb-2 px-1">
            <span className="flex items-center gap-1">
              <Megaphone className="w-3.5 h-3.5 text-indigo-400" />
              スポンサーリンク
            </span>
            <span className="text-[10px] bg-slate-800 light:bg-slate-200 px-1.5 py-0.5 rounded text-slate-400 light:text-slate-600 font-mono">
              {ad.badge}
            </span>
          </div>
          ...
          <div className={`w-full min-h-[200px] sm:min-h-[220px] bg-gradient-to-br ${ad.color} border ${ad.border} rounded-xl p-4 sm:p-5 flex flex-col justify-between transition-all group`}>
            <div className="space-y-2">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-lg bg-slate-900/80 light:bg-white border border-slate-700/60 light:border-slate-200 shadow-sm">
                  <AdIcon className={`w-5 h-5 ${ad.accent}`} />
                </div>
                <h4 className="font-bold text-slate-100 light:text-slate-900 text-sm sm:text-base leading-snug group-hover:text-indigo-200 light:group-hover:text-indigo-700 transition-colors">
                  {ad.title}
                </h4>
              </div>
            ...
            <div className="pt-3 mt-2 border-t border-slate-800/60 light:border-slate-200 flex items-center justify-between gap-3">
              <span className="text-[11px] text-slate-400 light:text-slate-500 font-mono">300×250 / 336×280 広告枠</span>
              <a
        ...
        {/* 下部: 軽音セッションお役立ちTips */}
        <div className="relative z-10 bg-slate-950/40 light:bg-slate-50 border border-slate-800/60 light:border-slate-200 rounded-xl p-3 flex items-start gap-2.5 text-slate-400 light:text-slate-500 text-xs leading-relaxed">
          <Lightbulb className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <div>
            <span className="font-semibold text-slate-300 light:text-slate-700">豆知識：</span>
            <span>{tip}</span>
          </div>
        </div>
      </div>
